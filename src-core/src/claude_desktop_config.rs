...
/// 检查 Claude provider 的 env 中 ANTHROPIC_*_MODEL 是否都是 Claude 安全 model id。
pub fn claude_provider_models_are_claude_safe(provider: &Provider) -> bool {
...
/// 根据当前 Claude provider 的 env 配置派生 Claude Desktop 模型路由。
pub fn suggested_claude_desktop_routes(
    provider: &Provider,
    ...
    fn add_route(
        ...
        let label_override = explicit_label_override.clone().or_else(|| {
            (!is_claude_safe_model_id(stripped_model)).then(|| stripped_model.to_string())
        });
