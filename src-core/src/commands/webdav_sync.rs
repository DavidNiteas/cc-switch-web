...
/// 上传本地快照到 WebDAV。
pub async fn webdav_sync_upload(state: &AppState) -> Result<Value, String> {
...
/// 从 WebDAV 下载快照并应用到本地。
pub async fn webdav_sync_download(state: &AppState) -> Result<Value, String> {
    let db = state.db.clone();
