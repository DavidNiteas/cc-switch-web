...
fn register_db_change_hook(conn: &Connection, callback: Option<DbChangeCallback>) {
    conn.update_hook(Some(
        move |action: Action, _database: &str, table: &str, _row_id: i64| match action {
...
impl Database {
    ...
    pub fn init() -> Result<Self, AppError> {
        Self::init_with_callback(None)
    }

    /// 初始化数据库连接并创建表，同时注册可选的变更回调。
    ///
    /// 回调在每次 INSERT/UPDATE/DELETE 时被调用，并传入受影响的表名。
    /// 桌面端可用它驱动自动同步与前端刷新。
    pub fn init_with_callback(change_callback: Option<DbChangeCallback>) -> Result<Self, AppError> {
        ...
        register_db_change_hook(&conn, change_callback);
    ...
    pub fn memory() -> Result<Self, AppError> {
        Self::memory_with_callback(None)
    }
