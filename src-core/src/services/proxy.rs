...
pub struct ProxyService {
    ...
    /// Platform，用于传递给 ProxyServer 以支持故障转移时的 UI 更新
    platform: Arc<RwLock<Option<Arc<dyn Platform>>>>,
    auth_state: Arc<ProxyAuthState>,
    switch_locks: SwitchLockManager,
    usage_log_callback: Arc<RwLock<Option<Arc<dyn Fn() + Send + Sync>>>>,
}
...
impl ProxyService {
    pub fn new(db: Arc<Database>) -> Self {
        Self {
            ...
            auth_state: Arc::new(ProxyAuthState::new()),
            switch_locks: SwitchLockManager::new(),
            usage_log_callback: Arc::new(RwLock::new(None)),
        }
    }

    pub fn set_platform(&self, platform: Arc<dyn Platform>) {
        futures::executor::block_on(async {
            *self.platform.write().await = Some(platform);
        });
    }

    pub fn set_usage_log_callback(&self, callback: Arc<dyn Fn() + Send + Sync>) {
        futures::executor::block_on(async {
            *self.usage_log_callback.write().await = Some(callback);
        });
    }

    /// 获取内部 `ProxyAuthState` 引用（共享 Copilot/Codex OAuth Manager）。
    pub fn auth_state(&self) -> &Arc<ProxyAuthState> {
        &self.auth_state
    }

    /// 获取当前注入的 Platform 句柄（可能为 None，比如 setup 早期）。
    pub async fn platform(&self) -> Option<Arc<dyn Platform>> {
    ...
    pub async fn start(&self) -> Result<ProxyServerInfo, String> {
        ...
        let usage_log_callback = self.usage_log_callback.read().await.clone();
        ...
        if enabled {
                ...
                crate::settings::get_effective_current_provider(&self.db, &app)
            {
                if let Ok(Some(provider)) = self.db.get_provider_by_id(&current_id, app_type_str) {
                    ...
                    {
                        if let Some(platform) = self.platform.read().await.as_ref() {
                            platform.emit_event(
