...
use crate::services::sql_helpers::{fresh_input_sql, INPUT_TOKEN_SEMANTICS_TOTAL};
...
pub fn find_model_pricing_row(
    ...
    // 1. 直接精确匹配（大小写不敏感）
    if let Some(row) = query_model_pricing_exact(conn, model_id)? {
        return Ok(Some(row));
    }
...
/// 判断 model_id 是否是"占位"模型，遇到这种模型时不应该输出 [USG-002] 警告。
///
/// 占位模型包括：空字符串、`unknown`、`unknown-model`、`placeholder-*`、
/// `*_session`（会话占位 provider 的模型）。
pub fn is_placeholder_pricing_model(model_id: &str) -> bool {
    ...
    matches!(lower.as_str(), "unknown" | "unknown-model")
        || lower.starts_with("placeholder-")
        || lower.starts_with("_codex_session")
        || lower.starts_with("_gemini_session")
        || lower.starts_with("_session")
}

/// SQL WHERE 片段：保留"有效"用量日志行——proxy 行保留，session 行仅在无
/// 匹配 proxy 行时保留（跨源去重，偏好 proxy 源）。
///
/// 调用方传入 `proxy_request_logs` 的别名（如 `"l"`），返回的片段可直接拼到
/// WHERE 子句中。匹配规则与 `has_matching_proxy_usage_log` 一致。
pub fn effective_usage_log_filter(log_alias: &str) -> String {
    let ds = data_source_expr(log_alias);
    let window = SESSION_PROXY_DEDUP_WINDOW_SECONDS;
    format!(
        "({ds} = 'proxy' OR NOT EXISTS (\
            SELECT 1 FROM proxy_request_logs l2 \
            WHERE COALESCE(l2.data_source, 'proxy') = 'proxy' \
              AND l2.app_type = {alias}.app_type \
              AND l2.status_code >= 200 AND l2.status_code < 300 \
              AND l2.input_tokens = {alias}.input_tokens \
              AND l2.output_tokens = {alias}.output_tokens \
              AND l2.cache_read_tokens = {alias}.cache_read_tokens \
              AND (l2.cache_creation_tokens = {alias}.cache_creation_tokens \
                   OR ({alias}.app_type IN ('codex','gemini','opencode') \
                       AND {alias}.cache_creation_tokens = 0)) \
              AND l2.created_at BETWEEN {alias}.created_at - {window} AND {alias}.created_at + {window} \
              AND (LOWER(l2.model) = LOWER({alias}.model) \
                   OR LOWER(l2.model) = 'unknown' \
...
fn query_model_pricing_exact(
    ...
    let sql = "SELECT input_cost_per_million, output_cost_per_million, \
               cache_read_cost_per_million, cache_creation_cost_per_million \
               FROM model_pricing WHERE LOWER(model_id) = LOWER(?1)";
    conn.query_row(sql, params![model_id], |row| {
        Ok((
            row.get::<_, String>(0)?,
            row.get::<_, String>(1)?,
            row.get::<_, String>(2)?,
...
/// 前缀匹配：用于 Claude Desktop 短路由 ID（如 `claude-haiku-4-5`）找到
/// 带日期后缀的完整 ID（如 `claude-haiku-4-5-20251001`）。
...
/// 生成清洗后的 model_id 候选列表，按优先级排序。
///
/// 顺序：原值 → 剥离命名空间 → 剥离 `@effort` 后缀 → 剥离 Bedrock 版本 →
/// 剥离 6 位日期后缀 → 剥离 Claude Desktop 非 Anthropic 前缀。
/// 每一步只对包含相应模式的输入产生新候选；重复候选会被去重。
fn model_pricing_candidates(model_id: &str) -> Vec<String> {
    let mut candidates: Vec<String> = Vec::new();
    let mut current = model_id.to_string();

    // 1. 剥离命名空间前缀（`anthropic/`, `moonshotai/`, `OpenAI/`, `google/`, `global.anthropic.`）
    if let Some(stripped) = strip_known_model_namespace(&current) {
    ...
    if let Some(idx) = current.rfind(':') {
        let stripped = current[..idx].to_string();
    ...
    // 3. 替换 `@` 为 `-`（如 `gpt-5.2-codex@low` → `gpt-5.2-codex-low`）
    if current.contains('@') {
        let replaced = current.replace('@', "-");
        push_unique_candidate(&mut candidates, &replaced);
        current = replaced;
    }
    ...
    // 5. 剥离 Bedrock 版本后缀（`-v1` 等）
    if let Some(stripped) = strip_bedrock_model_version_suffix(&current) {
        push_unique_candidate(&mut candidates, &stripped);
        current = stripped;
    }

    // 6. 剥离 6 位日期后缀（YYMMDD）
    if let Some(stripped) = strip_model_date_suffix(&current) {
        push_unique_candidate(&mut candidates, &stripped);
        current = stripped;
    }

    // 7. 剥离 `claude-` 非 Anthropic 前缀（如 `claude-gpt-5.5` → `gpt-5.5`）
    if let Some(stripped) = strip_claude_desktop_non_anthropic_prefix(&current) {
        push_unique_candidate(&mut candidates, &stripped);
    }
...
fn push_unique_candidate(candidates: &mut Vec<String>, value: &str) {
    ...
    if !candidates.iter().any(|c| c.to_ascii_lowercase() == lower) {
...
fn strip_reasoning_effort_suffix(model_id: &str) -> Option<String> {
    for suffix in ["-low", "-medium", "-high"] {
        if model_id.to_ascii_lowercase().ends_with(suffix) {
            return Some(model_id[..model_id.len() - suffix.len()].to_string());
        }
    }
    None
}

/// 剥离 Bedrock 版本后缀：`-v1` / `-v2` 等。
fn strip_bedrock_model_version_suffix(model_id: &str) -> Option<String> {
    let lower = model_id.to_ascii_lowercase();
    if let Some(idx) = lower.rfind("-v") {
        let tail = &model_id[idx + 2..];
        if !tail.is_empty() && tail.chars().all(|c| c.is_ascii_digit()) {
            return Some(model_id[..idx].to_string());
        }
    }
    None
}
...
///
/// 例：`claude-gpt-5.5` → `gpt-5.5`；`claude-sonnet-4-5` 不剥离（是 Anthropic 模型）。
