...
fn emit_auto_sync_status_updated(platform: &Arc<dyn Platform>, status: &str, error: Option<&str>) {
    ...
    platform.emit_event("webdav-sync-status-updated", payload);
}
...
async fn run_auto_sync_upload(
    ...
    match result {
        Ok(_) => {
            emit_auto_sync_status_updated(platform, "success", None);
            Ok(())
        }
        Err(err) => {
            persist_auto_sync_error(&mut sync_settings, &err);
            emit_auto_sync_status_updated(platform, "error", Some(&err.to_string()));
            Err(err)
        }
    }
}
...
pub fn start_worker(db: Arc<crate::database::Database>, platform: Arc<dyn Platform>) {
    ...
    tokio::spawn(async move {
        run_worker_loop(db, rx, platform).await;
    });
}
...
async fn run_worker_loop(
    ...
    while let Some(first_table) = rx.recv().await {
        ...
        if let Err(err) = run_auto_sync_upload(&db, &platform).await {
